...
import {
  CD_VER, retrieveData, getOsInfo, getChromedriverDir, getCurrentInstallChromeVersion,
  CD_MOST_RECENT_VER
} from './utils';
...
async function formatCdVersion (ver) {
  if (ver && _.toUpper(ver) !== LATEST_VERSION) {
    return ver;
  }

  const installVersion = await getCurrentInstallChromeVersion();
  if (!ver && !installVersion) {
    ver = CD_MOST_RECENT_VER;
  }
