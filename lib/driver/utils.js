...
import os from 'os';
import {OS, CPU, CHROMEDRIVER_DIR} from './constants';
...
const CD_BASE_DIR = (function() {
  if (CHROMEDRIVER_DIR) {
    if (path.isAbsolute(CHROMEDRIVER_DIR)) {
      return CHROMEDRIVER_DIR;
    } else {
      return path.join(process.cwd(), CHROMEDRIVER_DIR);
    }
  }
  return path.join(getModuleRoot(), 'chromedriver');
})();
const CHROMEDRIVER_DOWNLOAD_DIR = path.join(getModuleRoot(), 'config', 'download.json');
...
/**获取当前系统安装的chrome版本 */
async function getCurrentInstallChromeVersion() {
  const version = await ChromeVersion.getChromeVersion(false);
  if (version) {
    const mapVersion = CHROMEDRIVER_CHROME_MAPPING[version];
    // 存在driver版本
    if (mapVersion) {return version;}
    const versionFirst = version.split('.')[0];
    // 查找相邻的最新版本
    const newVesion = _.keys(CHROMEDRIVER_CHROME_MAPPING).find(k => k.split('.')[0] === versionFirst);
    return newVesion || version;
  }
  return version;
}
const CD_MOST_RECENT_VER = getMostRecentChromedriver();
const CD_VER =
...
async function getChromedriverBinaryPath(osName = getOsName()) {
  ...
  const version = await getCurrentInstallChromeVersion() || CD_VER;
  const pacthVersion = version.split('.');
  pacthVersion.pop();
  const pathSuffix = osName === OS.WINDOWS ? '.exe' : '';
  const paths = await fs.glob(`**/${CD_EXECUTABLE_PREFIX}*${pacthVersion.join('.')}*${pathSuffix}`, {
    cwd: rootDir,
    absolute: true,
    nocase: true,
    nodir: true,
...
export {
  ...
  getCurrentInstallChromeVersion,
  getOsName,
  CD_BASE_DIR,
  CD_VER,
  CD_MOST_RECENT_VER,
